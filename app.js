...
const defaultState = {
  ...
  displayName: "",
  entries: [
    { date: "2026-06-01", hours: 8, overtime: 0, overtimeUsed: 0, vacation: false, sick: false },
    { date: "2026-06-02", hours: 7.5, overtime: 0, overtimeUsed: 0, vacation: false, sick: false },
  ],
};
...
function entryFromForm() {
  if (selectors.entryVacation.checked || selectors.entrySick.checked) {
    return {
      start: "",
      end: "",
      breakMinutes: 0,
...
function monthEntries() {
  ...
  return state.entries.filter((entry) => entry.date.startsWith(current) && !entry.vacation && !entry.sick);
}
...
function buildExcelWorkbook() {
  ...
  return `<!doctype html>
    <html>
      ...
      <body>
        <table>
          <tr><td class="title" colspan="9">Arbeitszeiten Export</td></tr>
          <tr><td class="meta">Erstellt</td><td colspan="8">${escapeHtml(createdAt)}</td></tr>
          <tr><td class="meta">Name</td><td colspan="8">${escapeHtml(state.displayName || "")}</td></tr>
          <tr><td class="meta">Wochenstunden</td><td colspan="8">${exportNumber(state.weeklyHours)}</td></tr>
          <tr><td class="meta">Urlaubstage/Jahr</td><td colspan="8">${exportNumber(state.vacationTotal)}</td></tr>
          <tr><td class="meta">Bereits genommene Urlaubstage</td><td colspan="8">${exportNumber(state.vacationUsed)}</td></tr>
          <tr></tr>
          <tr>
            <th>Datum</th>
            <th>Start</th>
...
function openEntryModal(date) {
  ...
  setTimeFieldsDisabled(Boolean(entry?.vacation || entry?.sick));
...
function render() {
  ...
  const overtime = entries.reduce((sum, entry) => sum + Number(entry.overtime || 0) - Number(entry.overtimeUsed || 0), 0);
...
selectors.entryVacation.addEventListener("change", () => {
  if (selectors.entryVacation.checked) selectors.entrySick.checked = false;
  setTimeFieldsDisabled(selectors.entryVacation.checked || selectors.entrySick.checked);
});

selectors.entrySick.addEventListener("change", () => {
  if (selectors.entrySick.checked) selectors.entryVacation.checked = false;
  setTimeFieldsDisabled(selectors.entryVacation.checked || selectors.entrySick.checked);
});
